...
import edu.uci.ics.sctse.service.billing.models.CartUpdateRequestModel;
...
//        ServiceLogger.LOGGER.info("SESSION ID: " + sessionID);
        ...
        try
        {
            ...
            else
            {
                try
                {
                    if (CartDatabase.updateCartDB(requestModel.getEmail(), requestModel.getMovieId(), requestModel.getQuantity()))
                    {
                        ServiceLogger.LOGGER.info("Entry found, updating movieId/quantity");
                        resultCode = 3110;
                    }
                    else
                    {
